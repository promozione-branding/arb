...
export default function SubCategoryTable({ entry }) {
  ...
  const dimensionKeys =
    customColumns[subCategory.slug]?.dimensions ?? defaultDimensionKeys;

  const loadKeys =
    customColumns[subCategory.slug]?.loadRating ?? defaultLoadKeys;

  return (
    <div className="w-full mx-auto px-4 sm:px-8 md:px-30 mb- py-10">
      <nav className="mb-2 text-sm text-gray-400">
        {category.name} /{" "}
        <span className="text-gray-700">{subCategory.name}</span>
      </nav>

      <div className="flex-col flex md:flex-row items-start gap-3 justify-start md:items-center">
        {" "}
        <h2 className="mb-3 md:mb-6 text-2xl font-bold">{subCategory.name}</h2>
        <p className="mb-6 text-xs">{subCategory.smallDesc}</p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-600">
          ...
          {filteredProducts.length} bearings
        </p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {paginatedProducts.map((product) => (
              <tr key={product.slug} className="border-b hover:bg-blue-50">
